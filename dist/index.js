...
const categorize = (logs, scope) => {
    return (0, function_1.pipe)(IO.Do, IO.chain(() => IO.of(console.log(logs))), IO.chain(() => IO.of(console.log(scope))), IO.bind('feat', () => IO.of(logs.filter(filterLogBy(scope, 'feat')))), IO.bind('fix', () => IO.of(logs.filter(filterLogBy(scope, 'fix')))), IO.chain(({ feat, fix }) => IO.of({
        feat: feat,
        fix: fix
    })));
};
...
const generateDoc = (source) => {
    return (0, function_1.pipe)(Writer.tell(appendIfNeeded(source.feat, 'Features')), writer => w.chain(writer, () => Writer.tell(appendIfNeeded(source.fix, 'Bug Fixes'))), writer => Writer.execute(writer), docs => IO.of(docs));
};
exports.generateDoc = generateDoc;
const generateReleaseNote = (source) => {
    return IO.of(Array.reduce('', (acc, cur) => {
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        const program = (0, function_1.pipe)(TaskEither.Do, TaskEither.bind('commit_log', () => TaskEither.of(core.getInput('commit_log'))), TaskEither.bind('style', () => TaskEither.of(core.getInput('style'))), TaskEither.bind('scopes', () => TaskEither.of(Option.of(core.getInput('scopes')))), TaskEither.chain(({ commit_log, style, scopes }) => TaskEither.fromIO((0, classifier_1.categorize)(commit_log.split('\n'), Option.filter(s => s !== '')(scopes)))), TaskEither.bindTo('categorized'), TaskEither.chain(({ categorized }) => TaskEither.fromIO((0, generator_1.generateDoc)(categorized))), TaskEither.bindTo('docs'), TaskEither.chain(({ docs }) => TaskEither.fromIO((0, generator_1.generateReleaseNote)(docs))));
        Either.match(err => {
